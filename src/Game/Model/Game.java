...
public class Game {
	...
	//Il personaggio viene creato impostando la gravit� del gioco verso il basso
	private Game() {
		gravity = true;
	}
	...
	public void impostaMappa(String livello, boolean aggiorna) { // bool aggiorna protagonista o lo crea
	...
	public void restart() {
		...
		gravity=true;//Cos� il protagonista inizia con la gravit� sempre verso il basso
		if (player.vita == 0) {
	...
	public void GameOver() {
		...
		player.x = GestioneMappa.posInizialeX; //resettiamo la pos del player per non avere interferenza con ci� che c'era nella mappa
		player.y = GestioneMappa.posInzialeY;
	...
	private void nemicoIntelligente(Nemico e, int soffitto) { // se soffitto<0
		...
		if (player.y == e.y && e.segui) { //Gestione salto del nemico
			//La variabile salto indica di quanto il nemico si pu� ancora spostare in verticale
